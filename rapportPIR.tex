...
Following evaluation, the algorithm applies evolutionary operators to generate the next iteration of parameters. It utilizes tournament selection to choose parent genomes, naturally favoring individuals with lower worst-case costs. A uniform crossover operator blends the genes of the selected parents. To maintain genetic diversity and prevent premature convergence, two forms of mutation are applied. First, a local mutation introduces a scaling noise, with a mutation probability $p_m(g)$ that decays in as generations ($g$) progress, allowing for broad parameter search initially and fine-tuning in later stages:
\begin{equation}
	p_m(g) = 0.35 - 0.20 \left( \frac{g}{G} \right)
\end{equation}
Second, a global mutation operator completely replaces a small percentage of genes with new random values, ensuring the population can escape local minima in the complex, non-linear fitness landscape. Finally, an elitism mechanism guarantees that the absolute best-performing parameter set is preserved untouched into the subsequent generation, securing monotonic optimization progress throughout the training cycle.
